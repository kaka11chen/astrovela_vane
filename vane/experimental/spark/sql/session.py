...
# For us this is done inside of `vane.connect`, based on the passed in path + configuration
...
def _combine_data_and_schema(data: Iterable[Any], schema: StructType) -> list[vane.Value]:
...
class SparkSession:  # noqa: D101
    ...
    def catalog(self) -> "Catalog":  # noqa: D102
        if not hasattr(self, "_catalog"):
            from vane.experimental.spark.sql.catalog import Catalog
