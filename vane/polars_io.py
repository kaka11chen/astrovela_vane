...
def _predicate_to_expression(predicate: pl.Expr) -> vane.Expression | None:
    ...
    try:
        ...
        return vane.SQLExpression(sql_filter)
    except Exception:
...
def _pl_tree_to_sql(tree: _ExpressionTree) -> str:
    ...
    if node_type == "Scalar":
        ...
        if dtype == "String" or dtype == "StringOwned":
            ...
            return str(vane.ConstantExpression(string_val))
...
def duckdb_source(relation: vane.DuckDBPyRelation, schema: pl.schema.Schema) -> pl.LazyFrame:
