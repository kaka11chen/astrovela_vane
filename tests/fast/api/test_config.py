...
class TestDBConfig:
    ...
    def test_extension_setting(self):
        ...
        if not repository:
            return
        con = vane.connect(config={"TimeZone": "UTC", "autoinstall_extension_repository": repository})
        assert con.sql("select current_setting('TimeZone')").fetchone()[0] == "UTC"

    def test_unrecognized_option(self, duckdb_cursor):
        success = True
        try:
            vane.connect(":memory:", config={"thisoptionisprobablynotthere": "42"})
        except Exception:
            success = False
        assert not success
    ...
    def test_incorrect_parameter(self, duckdb_cursor):
        success = True
        try:
            vane.connect(":memory:", config={"default_null_order": "42"})
        except Exception:
            success = False
        assert not success
