    ...
    return value
		...
		std::thread worker([coro]() mutable {
			try {
				...
				// Convert to C++ while still holding the GIL so destruction of
				// Python objects and DECREFs happen safely under the GIL.
				int result = py::int_(res).cast<int>();
				...
				// Release and DECREF the captured coroutine object while still
				// holding the GIL so it is freed safely under the GIL instead of
				// during thread teardown.
				py::handle h = coro.release();
				PyObject *p = h.ptr();
				if (p) {
					Py_DECREF(p);
				}
