...
CopyUnnestSelectList(const std::vector<ExpressionRef> &select_list) {
...
UnnestNode::UnnestNode(NodeID node_id, PipelineNodeRef child, std::vector<ExpressionRef> select_list,
                       std::vector<LogicalType> output_types)
    : ctx_(InheritPipelineNodeContext(child, node_id, "Unnest")),
...
std::vector<PipelineNodeRef> UnnestNode::children() const {
	return {child_};
}

SubmittableTaskStream<WorkerTask> UnnestNode::produce_tasks(PlanExecutionContext &plan_context) {
	...
	return input_stream.pipeline_instruction(
	    ...
	    [select_list_ptr, output_types_ptr](DuckPhysicalPlanRef input_plan) -> DuckPhysicalPlanRef {
		    ...
		    auto select_list = CopyUnnestSelectList(*select_list_ptr);
		    ...
		    auto &unnest_op = input_plan->Make<duckdb::PhysicalUnnest>(std::move(types), std::move(select_list),
		                                                               estimated_cardinality);
		    unnest_op.children.push_back(old_root);
		    input_plan->SetRoot(unnest_op);
		    return input_plan;
	    },
	    plan_context.client_context());
}

std::vector<std::string> UnnestNode::multiline_display(bool /*verbose*/) const {
	std::string exprs;
	...
	return {std::string("Unnest: ") + exprs};
}
