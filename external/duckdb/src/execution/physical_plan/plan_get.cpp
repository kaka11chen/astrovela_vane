...
PhysicalOperator &PhysicalPlanGenerator::CreatePlan(LogicalGet &op) {
	...
	if (!op.function.projection_pushdown) {
		...
		cast_table_scan.named_parameters = std::move(op.named_parameters);
		cast_table_scan.input_table_types = std::move(op.input_table_types);
		cast_table_scan.input_table_names = std::move(op.input_table_names);
